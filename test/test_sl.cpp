...
sl_client_t* client;
sl_server_t* server;
void s_on_recv_data(char* buff, unsigned int size, void* conn, int channel)
{
...
void s_on_connected(void* conn, int status, int channel)
{
	cout << "s_on_connected ,channel:" << channel << " status:" << status << endl;

}

void s_on_close(void* conn, int channel)
{
	cout << "s_on_close ,channel:" << channel << endl;

}
//----------------------------------------------------------------------------
void c_on_recv_data(char* buff, unsigned int size, void* conn, int channel)
{
...
void c_on_connected(void* conn, int status, int channel)
{
	cout << "c_on_connected ,channel:" << channel << " status:" << status << endl;
	if (channel == TCP)
	{
		int conv = 0;  //just for test!
...
void c_on_close(void* conn, int channel)
{
...
int main()
{
	sl_client_create(&client);

	sl_client_cb(client, c_on_connected, c_on_recv_data, c_on_close);
	sl_client_init(client, "127.0.0.1", 6660, 6668);
	
	sl_server_create(&server);
	sl_server_cb(server, s_on_connected, s_on_recv_data, s_on_close);
	sl_server_start(server, "127.0.0.1", 6660, 6668,1024);
